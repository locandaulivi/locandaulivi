// ============================================================
//  LOCANDA ULIVI – Dati del sito
//  I campi con (*) sono modificabili dal pannello Admin
// ============================================================
...
const SITE_DATA_DEFAULT = {
  ristorante: {
    ...
    via:         "Località Alpi 3",       // *
    cap:         "37010 Cavaion Veronese (VR), Italia",  // *
  ...
  pranzo_lavoro: {
    ...
    prezzo:      "€ 15,00",          // *
    descrizione: "Menu completo pensato per chi vuole pranzare bene senza perdere tempo. Dalle 12:00 alle 14:30.",  // *
    incluso: [   // *
  ...
  menu: {
    ...
    primi: [  // *
      ...
      { nome: "Tagliatelle al ragù di cinghiale", prezzo: "15", descrizione: "Tagliatelle fresche con lento ragù di cinghiale e bacche di ginepro" }
    ],
    secondi: [  // *
  ...
  galleria: {
    locale: [],   // array di { src, alt }  *
    piatti: []    // array di { src, alt }  *
  },
  stemma: null    // path immagine stemma  *
};

// ---- Load saved data or use defaults ----
function loadData() {
  try {
    const saved = localStorage.getItem('locanda_ulivi_data');
    if (saved) return JSON.parse(saved);
  } catch(e) {}
  return JSON.parse(JSON.stringify(SITE_DATA_DEFAULT));
}

function saveData(data) {
  localStorage.setItem('locanda_ulivi_data', JSON.stringify(data));
}
